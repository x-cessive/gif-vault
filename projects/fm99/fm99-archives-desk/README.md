# fm99 — fm99-archives-desk

The Now Spinning 99.9 FM archives desk: 30 states, one constant department room.
...
Character: mole (spectacles). Room: archive stacks — filing cabinets, boxes, card catalog, ledgers.
Personality: near-sighted meticulous burrower; spectacles show the mood.

- `gifs/` — the 30 GIFs (`fm99_archives_<state>.gif`)
...
Pipeline: `~/workspace/gif-station/fm99_archives_set.py` (built on
`fm99_archives_pilot.py` + `office_rig.py`).
