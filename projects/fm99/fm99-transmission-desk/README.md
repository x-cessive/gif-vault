# fm99 — fm99-transmission-desk

The Now Spinning 99.9 FM transmission desk: 30 states, one constant department room.
...
Character: carrier pigeon (antenna headset). Room: transmitter room — tower diagram, signal meters, blinking beacon.
Personality: dutiful messenger; the head-bob is the soul of it.

- `gifs/` — the 30 GIFs (`fm99_transmission_<state>.gif`)
...
Pipeline: `~/workspace/gif-station/fm99_transmission_set.py` (built on
`fm99_transmission_pilot.py` + `office_rig.py`).
